...
if (isset($_POST['report'])) {
   ...
   $date =  $_POST['date_to_check'];
  

  $query = "SELECT * FROM booking WHERE  check_in LIKE '%$date%'";
  $result = mysqli_query($connection, $query);
  if (mysqli_num_rows($result) > 0) {
      echo("    <table class='table' style='margin:auto; width:50%; padding-top:10px'>
      <head>
      <tr>
      <th scope='cole'> # </th>
      <th scope='cole'> Totoal Reservations </th>
      <th scope='cole'> Total Booking </th>
      <th scope='cole'> Total Earnings</th>
      <th scope='cole'> Pending Earnings</th>
      </tr>
      </head>
      <tbody>");
...
");
    ...
    echo("<h2 style='text-align:center; color:red'> No Report To Submit </h2>");

//$bookingnum = mysqli_fetch_assoc($result);
...
$password = md5($password);
 ...
 $query = "INSERT INTO user SET name='$name', username='$username', password='$password', email='$email', role_id='$role_id'" ;
